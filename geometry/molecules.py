...
class Molecule(object):
    ...
    def addatomsFromLists(self, atomlist, coordslist):
        ...
        zipped = zip(atomlist, coords)
        for x, y in zipped:
            self.addatom(x, y, unit='angstrom')
        return

    def readGauConnect(self, conntystring):
        ...
        for atom1 in self:  # atom1: atom obj
            for atom2 in atom1.neighbors:  # atom2: obj
                if atom2 is atom1:
                    continue
                for atom3 in atom2.neighbors:  # atom3: obj
...
class Atom(object):
    def __init__(self, mole, atomnoORelement, coords,
        ...
        if isinstance(atomnoORelement, int):
            ...
            try:
                self._element = periodictable.elements[self._atomno].symbol
            except KeyError:
            ...
            try:
                self._atomno = getattr(periodictable,self._element).number
            except KeyError:
                logging.critical("Error when adding atom: "
